...
```{r}
az_mine_buffer<-st_buffer(az_mine, dist=5000)
...
```{r}
dem_diff_crs<-st_crs(dem_diff)

az_mine_buffer_proj<-az_mine_buffer%>%
  st_transform(dem_diff_crs$epsg)

#checking to see if the projection worked
st_crs(az_mine_buffer_proj)
...
#Your code here
?mask
dem_diff_invmask<-dem_diff%>%
  raster::mask(az_mine_buffer_proj,inverse=TRUE)
plot(dem_diff_invmask)
```
...
az_mine_proj<-az_mine%>%
  st_transform(dem_diff_crs$epsg)

dem_diff_invmask2<-dem_diff%>%
  raster::mask(az_mine_proj,inverse=TRUE)
plot(dem_diff_invmask2)
...
unmined_dem<-getValues(dem_diff_invmask2)
mined_dem <- getValues(dem_diff)
?getValues
...
#Plot of Dem Value desities                      
ggplot(mined_table,aes(dem_values,fill=type))+
  geom_density()
?geom_density
